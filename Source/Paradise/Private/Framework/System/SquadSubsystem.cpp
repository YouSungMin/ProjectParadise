...
void USquadSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	UE_LOG(LogSquad, Warning, TEXT("❌ [SquadSubsystem] Initialize 실행"));
...
void USquadSubsystem::SetFamiliarToSlot(int32 SlotIndex, FName NewFamiliarID)
{
	...
	{
		UE_LOG(LogSquad, Warning, TEXT("❌ [SquadSubsystem] 유효하지 않은 퍼밀리어 슬롯 인덱스입니다: %d (0~4만 가능)"), SlotIndex);
		return;
	}
	...
	if (!NewFamiliarID.IsNone())
	{
		...
		{
			UE_LOG(LogSquad, Error, TEXT("❌ [SquadSubsystem] 편성 실패! '%s'는 유효하지 않은 퍼밀리어 ID입니다."), *NewFamiliarID.ToString());
			return;
		}
	}
	...
	UE_LOG(LogSquad, Log, TEXT("✅ [SquadSubsystem] 슬롯 %d에 퍼밀리어 %s 편성 완료"), SlotIndex, *NewFamiliarID.ToString());
...
void USquadSubsystem::SetPlayerToSlot(int32 SlotIndex, FName NewPlayerID)
{
	...
	{
		UE_LOG(LogSquad, Warning, TEXT("❌ [SquadSubsystem] 유효하지 않은 슬롯 인덱스입니다: %d"), SlotIndex);
		return;
	}
	...
	if (!NewPlayerID.IsNone())
	{
		...
		{
			UE_LOG(LogSquad, Error, TEXT("❌ [SquadSubsystem] 편성 실패! '%s'는 유효하지 않은 캐릭터 ID입니다."), *NewPlayerID.ToString());
			return;
		}
	}
	...
	UE_LOG(LogSquad, Log, TEXT("✅ [SquadSubsystem] 슬롯 %d에 플레이어 %s 편성 완료"), SlotIndex, *NewPlayerID.ToString());
...
void USquadSubsystem::LoadFromSaveGame(UParadiseSaveGame* SaveGameObj)
{
	...
	UE_LOG(LogSquad, Log, TEXT("✅ [SquadSubsystem] 세이브 파일에서 편성 정보를 성공적으로 불러왔습니다."));
}
...
void USquadSubsystem::SaveToSaveGame(UParadiseSaveGame* SaveGameObj) const
{
	...
	UE_LOG(LogSquad, Log, TEXT("💾 [SquadSubsystem] 세이브 객체에 편성 정보를 기록했습니다."));
}
